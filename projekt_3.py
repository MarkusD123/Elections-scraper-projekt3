...
def scrap_data_write_to_csv(main_url, csv_file):
    ...
    vysledny_soubor_csv = open(csv_file, encoding='utf-8', newline='', mode="w")
    ...
    for i in range(0, x): #vyextrahovanie do listu a nasledny zapis kazdeho riadku do csv suboru (okrem hlavicky)
        ...
        vysledny_soubor_csv = open(csv_file, encoding='utf-8', newline='', mode="a")
        zapisovac = csv.writer(vysledny_soubor_csv)
        zapisovac.writerow(row_obec)
        vysledny_soubor_csv.close()
    print('Ukoncuji election scraper projekt_3')

main_url = 'https://volby.cz/pls/ps2017nss/ps32?xjazyk=CZ&xkraj=12&xnumnuts=7103'  # Argument 1: adresa Prostejov
csv_file = "vysledky_prostejov.csv" #Argument 2: - nazov csv suboru
scrap_data_write_to_csv(main_url, csv_file)
